...
def test_orders_list_with_pagination_filters():
    response = client.get("/api/v1/orders?page=1&page_size=10&search=TRK001")
...
def test_order_detail_events_assign_cancel_flow():
    order_id = client.get("/api/v1/orders").json()["items"][0]["id"]
    ...
    assign = client.post(f"/api/v1/orders/{order_id}/assign", json={"drone_id": "DR-2"})
    assert assign.status_code == 200

    cancel = client.post(f"/api/v1/orders/{order_id}/cancel")
    assert cancel.status_code == 200


def test_jobs_and_tracking_views():
    jobs = client.get("/api/v1/jobs?active=true")
