...
- **Health endpoint** at `GET /health` (non-blocking downstream status for `fleet_api` and `gcs_bridge`).
...
Health endpoint now includes non-blocking downstream dependency status:
- `fleet_api`: `ok` / `degraded` / `down`
